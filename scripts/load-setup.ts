/**
 * Load Setup Script - Large Volume Test Data
 * Creates a high-volume test environment for load/stress testing
 *
 * Usage:
 *   npx tsx scripts/load-setup.ts [local|staging]
 *
 ...
 *
 * Phase 2: High-Volume Data
 * - 500 Sales Orders
 * - 400 Purchase Orders
 * - ~320 Goods Inwards (80% fulfillment of purchase orders)
 * - 150 Goods Transfers (warehouse → factory)
 * - 20 Goods Converts (raw materials → finished goods at factories)
 * - 300 Goods Transfers total (factory → warehouse, 150 more)
 * - ~300 Goods Outwards (60% fulfillment of sales orders)
 * - ~400 Sales Invoices (80% of sales orders)
 * - ~320 Purchase Invoices (80% of purchase orders)
 * - ~90 Sales Adjustment Notes
 * - ~72 Purchase Adjustment Notes
 * - ~320 Payment Receipts
 * - ~256 Payment Made
 * - 50 QR Batches (10 per warehouse)
 */
...
/**
 * QR batches config with higher batch count per warehouse for load testing
 * 10 batches/warehouse × 5 warehouses = 50 total (vs 15 in basic-setup)
 */
const LOAD_QR_BATCHES_CONFIG = {
  ...QR_BATCHES_CONFIG,
  batchesPerWarehouse: 10,
} as unknown as typeof QR_BATCHES_CONFIG;
...
async function runSetup() {
  console.log("🚀 Starting Load Setup Script - High Volume Test Data\n");
  ...
  try {
    ...
    // Use a fixed year for data generation
    const currentYear = 2025;

    // ========================================================================
    // PHASE 2: HIGH-VOLUME DATA GENERATION
    // ========================================================================

    console.log("\n📋 PHASE 2: High-Volume Data Generation\n");

    // Step 8: Sales Orders (500) — finished products only
    ...
    // Step 9: Purchase Orders (400) — 80% raw, 20% finished
    ...
    // Step 10: Goods Inwards (~320, 80% of purchase orders)
    ...
    // Step 11: Mark 50% of purchase orders as completed (post-inwards)
    ...
    // Step 12: Goods Transfers — Warehouse to Factory (150 transfers)
    ...
    // Step 14: Goods Transfers — Factory to Warehouse (300 total, 150 more)
    const transfersToWarehouse = await generateGoodsTransfers(
      ...
      GOODS_TRANSFERS_CONFIG,
      {
        totalTransfers: 300, // Total cumulative (150 + 150)
        direction: "factory_to_warehouse",
        year: currentYear,
      },
    );

    // Step 15: Goods Outwards (~300, 60% of sales orders)
    ...
    // Step 16: Mark 50% of sales orders as completed (post-outwards)
    const salesOrderCompletionRecords = Object.fromEntries(
      salesOrders
        .filter(() => Math.random() < 0.5)
        .map((so) => [so.id, { status: "completed" }]),
    );
    await updateRecords(supabase, "sales_orders", salesOrderCompletionRecords);

    // Step 17: Fetch charge ledgers (reusable for both invoice types)
    const chargeLedgers = await fetchChargeLedgers(supabase, companyId);

    // Step 18: Sales Invoices (~400, 80% of sales orders, with additional charges inline)
    ...
    // Step 19: Purchase Invoices (~320, 80% of purchase orders, with additional charges inline)
    ...
    // Step 21: Sales Adjustment Notes (~90, 22.5% of eligible sales invoices)
    ...
    // Step 22: Purchase Adjustment Notes (~72, 22.5% of eligible purchase invoices)
    ...
    // Step 24: Payment Receipts (~320, 80% of sales invoices with outstanding)
    ...
    // Step 25: Payment Made (~256, 80% of purchase invoices with outstanding)
    ...
    // Step 26: QR Batches (50 total, 10 per warehouse)
    const qrBatches = await generateQRBatches(
      supabase,
      companyId,
      warehouses,
      userId,
      LOAD_QR_BATCHES_CONFIG,
    );
    ...
    console.log("\n" + "=".repeat(80));
    console.log("✅ LOAD SETUP COMPLETE - HIGH VOLUME TEST DATA");
    ...
    console.log(
      `   Purchase Order completions: ${Object.keys(purchaseOrderCompletionRecords).length}`,
    );
    console.log(
    ...
    console.log(
      `   Sales Order completions: ${Object.keys(salesOrderCompletionRecords).length}`,
    );
