      ...
      if (stockError || !stockUnits || stockUnits.length === 0) {
        ...
        const testOutwards = [
          ...
          },
          {
            ...
            partner_id: customerId1,
            outward_type: "other",
            other_reason: "Samples for new collection",
      ...
      if (outwardsError || !goodsOutwards || goodsOutwards.length === 0) {
        ...
        if (invProdError || !invoiceProducts || invoiceProducts.length === 0) {
          ...
          if (inwardsError || !goodsInwards || goodsInwards.length === 0) {
            ...
            // ================================================
            console.log("💵 Adding additional charges to invoices...\n");
            ...
            if (!chargeLedgers || chargeLedgers.length === 0) {
              ...
              // Add charges to sales invoices (75% of them)
              for (const invoiceId of salesInvoiceIds) {
                if (Math.random() > 0.75) continue; // 75% get charges

                const charges = [];

                // Freight Outward (50% chance, flat amount)
                if (freightOutwardLedger && Math.random() < 0.5) {
                  charges.push({
                    company_id: companyId,
                    invoice_id: invoiceId,
                    ledger_id: freightOutwardLedger.id,
                    charge_type: "flat_amount",
                    charge_value: 500 + Math.floor(Math.random() * 1500), // ₹500-2000
                    sequence_order: charges.length,
                  });
                }
                ...
                if (agentCommissionLedger && Math.random() < 0.5) {
                  charges.push({
                    ...
                    charge_value: 2 + Math.random() * 3, // 2-5%
                    sequence_order: charges.length,
                  });
                }
              ...
              console.log(
                `   ✅ Added ${chargesCreated} charges to sales invoices`,
              );

              chargesCreated = 0;

              // Add charges to purchase invoices (65% of them)
              for (const invoiceId of purchaseInvoiceIds) {
                if (Math.random() > 0.65) continue; // 65% get charges

                const charges = [];

                // Freight Inward (60% chance, flat amount)
                if (freightInwardLedger && Math.random() < 0.6) {
                  charges.push({
                    company_id: companyId,
                    invoice_id: invoiceId,
                    ledger_id: freightInwardLedger.id,
                    charge_type: "flat_amount",
                    charge_value: 800 + Math.floor(Math.random() * 2200), // ₹800-3000
                    sequence_order: charges.length,
                  });
                }

                // Labour Charges (35% chance, flat amount)
                if (labourLedger && Math.random() < 0.35) {
                  charges.push({
                    company_id: companyId,
                    invoice_id: invoiceId,
                    ledger_id: labourLedger.id,
                    charge_type: "flat_amount",
                    charge_value: 500 + Math.floor(Math.random() * 1000), // ₹500-1500
                    sequence_order: charges.length,
                  });
                }
              ...
              console.log(
                `   ✅ Added ${chargesCreated} charges to purchase invoices\n`,
              );
            }
              ...
              if (openInvError || !openInvoices || openInvoices.length === 0) {
                ...
                const receiptData = [
                  // Fully settled
                  {
                    ...
                    percentage: 100,
                    mode: paymentModes[1], // UPI
                    transaction_id: "UPI402912345678",
                    vpa: "customer@paytm",
                  },
                  {
                    invoice: salesInvoicesForPayment[1],
                    percentage: 100,
                    mode: paymentModes[2], // NEFT
                    transaction_id: "NEFT987654321",
                  },
                  {
                    invoice: salesInvoicesForPayment[2],
                    percentage: 100,
                    mode: paymentModes[5], // Card
                    transaction_id: "CARD456789123",
                    card_last_four: "4532",
                  },
                  // Partially paid
                  {
                    ...
                    percentage: 60,
                    mode: paymentModes[3], // Cheque
                    instrument_number: "123456",
                    instrument_date: new Date().toISOString().split("T")[0],
                    instrument_bank: "HDFC Bank",
                    instrument_branch: "MG Road",
                    instrument_ifsc: "HDFC0001234",
                  },
                  {
                    invoice: salesInvoicesForPayment[4],
                    percentage: 50,
                    mode: paymentModes[4], // RTGS
                    transaction_id: "RTGS111222333",
                  },
                  {
                    invoice: salesInvoicesForPayment[5],
                    percentage: 70,
                    mode: paymentModes[6], // IMPS
                    transaction_id: "IMPS987654321",
                  },
                  // Advance payment (no invoice)
                  {
                    invoice: null,
                    amount: 25000,
                    mode: paymentModes[0], // Cash
                  },
                ];
                ...
                const vendorPaymentData = [
                  ...
                  },
                  {
                    ...
                    instrument_date: new Date().toISOString().split("T")[0],
                    instrument_bank: "ICICI Bank",
                    instrument_branch: "Koramangala",
                    instrument_ifsc: "ICIC0006789",
                    tds: false,
                  },
                  // Partially paid
                  {
                  ...
                  // With TDS
                  {
                    ...
                    percentage: 100,
                    mode: paymentModes[7], // Demand Draft
                    instrument_number: "DD445566",
                    instrument_date: new Date().toISOString().split("T")[0],
                    instrument_bank: "State Bank of India",
                    instrument_branch: "Main Branch",
                    instrument_ifsc: "SBIN0001234",
                    tds: true,
                    tds_rate: 1,
                  },
                  {
