...
interface GoodsOutwardSelectionDialogProps {
  open: boolean;
...
export function GoodsOutwardSelectionDialog({
  open,
  onOpenChange,
  orderNumber,
  onInvoiceFromMovements,
  onInvoiceFullOrder,
}: GoodsOutwardSelectionDialogProps) {
  const [selectedIds, setSelectedIds] = useState<string[]>([]);

  // Fetch goods outwards for this sales order
  const { data: outwardsData } = useGoodsOutwardsBySalesOrder(orderNumber);
  const movements = outwardsData?.data || [];
  ...
  return (
    <ResponsiveDialog
      ...
      description={`Select outward movements to include in the invoice for SO-${orderNumber}`}
      footer={
      ...
      {movements.length > 0 ? (
        <div className="flex flex-col gap-3">
          {movements.map((movement) => {
            ...
            const movementDate = formatDateDisplay(
              new Date(movement.outward_date),
            );
            const productsSummary = getOutwardProductsSummary(movement);
        ...
        <div className="flex items-center justify-center py-12">
          <div className="text-center text-gray-500">
            <p className="text-sm">
              No outward movements found for this order.
            </p>
