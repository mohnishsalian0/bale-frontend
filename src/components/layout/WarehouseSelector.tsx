...
export default function WarehouseSelector({
  ...
  return (
    <>
      ...
      {/* Dropdown Panel */}
      <div
        className={`fixed left-0 right-0 bg-background-100 shadow-sm z-40 transition-transform duration-300 ease-in-out ${
          isOpen ? 'top-[68px] translate-y-0' : 'top-[68px] pointer-events-none -translate-y-full'
        }`}
      >
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 p-4 transition-colors cursor-pointer ${
                    index !== warehouses.length - 1 ? 'border-b border-gray-200' : ''
                  } ${
                    isSelected
                      ? 'bg-primary-100'
