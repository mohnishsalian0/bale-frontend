...
export function getPaymentAllocationSummary(
  allocations: PaymentAllocationListView[],
): string {
  ...
  const againstRefAllocations = allocations.filter(
    (allocation) =>
      allocation.allocation_type === "against_ref" && allocation.invoice,
  );
  ...
  const summaryParts = againstRefAllocations.map((allocation) => {
    const invoiceNumber =
      allocation.invoice?.invoice_number || "Unknown Invoice";
