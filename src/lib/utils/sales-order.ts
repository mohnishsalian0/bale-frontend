...
export function calculateCompletionPercentage(items: OrderItem[]): number {
	...
	const totalDispatched = items.reduce((sum, item) => sum + (item.dispatched_quantity || 0), 0);
 ...
 */
export function getProductSummary(products: Array<{ name: string; quantity: number }>): string {
