...
-- =====================================================
-- GOODS TRANSFER GUARDS
-- =====================================================

-- Unified function to prevent editing goods transfers in invalid states
CREATE OR REPLACE FUNCTION prevent_goods_transfer_edit()
RETURNS TRIGGER AS $$
BEGIN
    -- Rule 1: Cannot edit if status is completed
    IF OLD.status = 'completed' THEN
        RAISE EXCEPTION 'Cannot edit goods transfer - transfer is completed';
    END IF;

    -- Rule 2: Cannot edit if status is cancelled
    IF OLD.status = 'cancelled' THEN
        RAISE EXCEPTION 'Cannot edit goods transfer - transfer is cancelled';
    END IF;

    -- Rule 3: Cannot edit if deleted
    IF OLD.deleted_at IS NOT NULL THEN
        RAISE EXCEPTION 'Cannot edit goods transfer - transfer is deleted';
    END IF;
...
CREATE TRIGGER trigger_prevent_goods_transfer_edit
    BEFORE UPDATE ON goods_transfers
    FOR EACH ROW
    EXECUTE FUNCTION prevent_goods_transfer_edit();

COMMENT ON FUNCTION prevent_goods_transfer_edit() IS 'Prevents editing goods transfers that are completed, cancelled, or deleted.';

-- Prevent goods transfer deletion in invalid states
CREATE OR REPLACE FUNCTION prevent_goods_transfer_delete()
RETURNS TRIGGER AS $$
BEGIN
    -- Rule 1: Cannot delete if status is completed (use soft delete)
    IF OLD.status = 'completed' THEN
        RAISE EXCEPTION 'Cannot delete completed goods transfer. Use soft delete (deleted_at) instead';
    END IF;

    -- Rule 2: Cannot delete if status is cancelled (use soft delete)
    IF OLD.status = 'cancelled' THEN
        RAISE EXCEPTION 'Cannot delete cancelled goods transfer. Use soft delete (deleted_at) instead';
    END IF;

    -- Rule 3: Cannot delete if deleted (already soft deleted)
    IF OLD.deleted_at IS NOT NULL THEN
        RAISE EXCEPTION 'Cannot delete goods transfer - already soft deleted';
    END IF;

    -- Note: CAN delete in_transit transfers - cascade delete will handle transfer items
    -- Transfer items have ON DELETE CASCADE FK to goods_transfers

    RETURN OLD;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trigger_prevent_goods_transfer_delete
    BEFORE DELETE ON goods_transfers
    FOR EACH ROW
    EXECUTE FUNCTION prevent_goods_transfer_delete();

COMMENT ON FUNCTION prevent_goods_transfer_delete() IS 'Prevents deleting completed, cancelled, or soft-deleted goods transfers. Allows deletion of in_transit transfers (cascade handled by FK).';
