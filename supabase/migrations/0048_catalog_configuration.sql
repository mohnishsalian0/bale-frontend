...
TO authenticated
USING (
    company_id = get_jwt_company_id() AND authorize('business.catalog.read')
);
...
TO authenticated
WITH CHECK (
    company_id = get_jwt_company_id() AND authorize('business.catalog.create')
);
...
TO authenticated
USING (
    company_id = get_jwt_company_id() AND authorize('business.catalog.update')
)
WITH CHECK (
    company_id = get_jwt_company_id() AND authorize('business.catalog.update')
);
...
TO authenticated
USING (
    company_id = get_jwt_company_id() AND authorize('business.catalog.delete')
);
