...
TO authenticated
USING (
    company_id = get_jwt_company_id() AND authorize('business.products.read')
);
...
TO authenticated
WITH CHECK (
    company_id = get_jwt_company_id() AND authorize('business.products.create')
);
...
TO authenticated
USING (
    company_id = get_jwt_company_id() AND authorize('business.products.update')
)
WITH CHECK (
    company_id = get_jwt_company_id() AND authorize('business.products.update')
);
...
TO authenticated
USING (
    company_id = get_jwt_company_id() AND authorize('business.products.delete')
);
