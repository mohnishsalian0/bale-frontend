...
USING (
    ...
    authorize('accounting.ledgers.read') AND
    deleted_at IS NULL
);
...
WITH CHECK (
    company_id = get_jwt_company_id() AND
    authorize('accounting.ledgers.create')
);
...
USING (
    company_id = get_jwt_company_id() AND
    authorize('accounting.ledgers.update') AND
    deleted_at IS NULL
)
WITH CHECK (
    company_id = get_jwt_company_id() AND
    authorize('accounting.ledgers.update')
);
...
USING (
    company_id = get_jwt_company_id() AND
    authorize('accounting.ledgers.delete') AND
    deleted_at IS NULL
);
